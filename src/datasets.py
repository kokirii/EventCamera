...
class Sequence(Dataset):
    ...
    def __len__(self):
        return len(self.timestamps_flow) - 1  # 変更点：最後のフレームは次のフレームがないため除外
    ...
    def get_data(self, index) -> Dict[str, any]:
        ...
        ts_start2 = self.timestamps_flow[index + 1] - self.delta_t_us
        ts_end2 = self.timestamps_flow[index + 1]  # 変更点：次のフレームのタイムスタンプを取得
        ...
        event_data2 = self.event_slicer.get_events(ts_start2, ts_end2)  # 変更点：次のフレームのイベントデータを取得
        ...
        if self.voxel_grid is None:
            ...
            event_representation = torch.cat((event_representation1, event_representation2), dim=0)  # 変更点：2つのフレームを結合
